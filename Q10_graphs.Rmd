...
mod <- gls(data=alldata, Q10T ~ 1 + midDepth*landCover + MAP*midDepth + MAT*midDepth , na.action = na.omit)
mod2 <- lmer(data=alldata, Q10T ~ 1 + midDepth*landCover + MAP*midDepth + MAT*midDepth  + (1|uniqueSite), na.action = na.omit)
mod3 <- lmer(data=alldata, Q10T ~ 1 +midDepth*landCover + MAP*midDepth + MAT*midDepth  + (1|citationKey), na.action = na.omit)
mod4 <- lmer(data=alldata,Q10T ~ 1 + midDepth*landCover + MAP*midDepth + MAT*midDepth  + (1|citationKey/uniqueSite), na.action = na.omit)
...
#LandCover by depth
mylist <- list(midDepth=c(5,15,30,50,100))
emmip(mod4,~midDepth|landCover,at=mylist, CIs=TRUE)

emtrends(mod4, pairwise~ landCover, var = "midDepth")

#MAT partial residual plot

library(ggeffects)
pr<-ggpredict(mod4, "MAT [all]")
plot(pr, add.data = TRUE)
plot(pr, residuals = TRUE, residuals.line = TRUE) #is absolutely a linear effect

pr<-ggpredict(mod4, c("midDepth", "landCover"))
...
##Model
```{r}
print("number of data points with all variables (environmental model: midDepth, landCover, nitrogen, MAP, MAT, pH)")
length(which(!is.na(alldata$midDepth) & !is.na(alldata$nitrogenPercent) & !is.na(alldata$carbonPercentAll)& !is.na(alldata$pH)&!is.na(alldata$microbialBiomass2))) #224, 103 with microbial biomass

#look for optimal random effects using transformed Q10
mod <- gls(data=alldata, Q10T ~ 1 + midDepth*nitrogenPercent + carbonPercentAll*midDepth + pH*midDepth , na.action = na.omit)
mod2 <- lmer(data=alldata, Q10T ~ 1 + midDepth*nitrogenPercent + carbonPercentAll*midDepth + pH*midDepth  + (1|uniqueSite), na.action = na.omit)
mod3 <- lmer(data=alldata, Q10T ~ 1 +midDepth*nitrogenPercent + carbonPercentAll*midDepth + pH*midDepth  + (1|citationKey), na.action = na.omit)
mod4 <- lmer(data=alldata,Q10T ~ 1 + midDepth*nitrogenPercent + carbonPercentAll*midDepth + pH*midDepth  + (1|citationKey/uniqueSite), na.action = na.omit)
#Some predictor variables are on very different scales: consider rescaling
...
#graphical exploration of residuals
F_Final <- fitted(mod3)
R_Final <- residuals(mod3, type = "pearson", scaled = TRUE) #type="response" 
#no difference in graphs with pearson vs response
...
#depth
mylist <- list(midDepth=c(5,15,30,50,100))
emmip(mod3,~midDepth,at=mylist, CIs=TRUE)

#nitrogen by depth
mylist <- list(nitrogenPercent=c(Na,N,Nb))
emmip(mod3,~nitrogenPercent, at=mylist, CIs=TRUE)

Na <- (mean(alldata$nitrogenPercent, na.rm=T) + sd(alldata$nitrogenPercent, na.rm=T))
N <- (mean(alldata$nitrogenPercent, na.rm=T))
Nb <- 0.05

mylist <- list(midDepth=c(5,15,30,50,100),nitrogenPercent=c(Na,N,Nb)) 
emmip(mod3,midDepth~nitrogenPercent,at=mylist)

#pH by depth
Na <- round(mean(alldata$pH, na.rm=T) + sd(alldata$pH, na.rm=T))
N <- round(mean(alldata$pH, na.rm=T))
Nb <- round(mean(alldata$pH, na.rm=T) - sd(alldata$pH, na.rm=T))

mylist <- list(midDepth=c(5,15,30,50,100),pH=c(Na,N,Nb)) 
